...
A lot of spreadsheet usage is centered around copy-pasting formulas around. Specifically, replicating formulas across a range of data. Here's an example: I have a list of numbers that I want to multiply by five.
...
This works because of relative cell references under the hood. When a relative reference (`A2`) is copied to another cell it adjusts according to the new location. Effectively the reference is an offset and does not refer to a specific cell. An absolute cell reference (`$A$2`) is tied to a specific cell, so when you copy the formula it stays the same.[^microsoft]

Copy-pasting aka "filling down" makes up a big part of spreadsheet magic. A user only needs to write a formula in terms of a single item and then fill it down. Copy pasting however means duplication, which means repeated manual work.
...
In general, formula replication is known to be error-prone and easy to get wrong. [emphasise in two sentences how big is this problem.]
...
Soon after in Multiplan – Microsoft's contender to Visicalc – we notice a special notation for relative references for the first time. Multiplan did not have the same kind of referencing. Instead, it had this[^multiplan]:
...
<!-- We start our investigation by asking: Why do people copy-paste formulas in the first place?  -->
...
Relative references enable users to do more elaborate list operations as well: A formula can refer to the cell above it, and filling down such a formula produces what's called a `scan`[^scan]. A common use case for this is calculating cumulative totals[^elastic-sdf-example].

However the fact that "these cells are a collection" is captured in the user's mind, not on the sheet. Formulas are merely duplicated across clusters of cells without any constraints on regularity. As things change over time – new items are added, formulas are tweaked and the sheet exchanges hands – this implicit knowledge of the sheet's structure manifests as errors.

This points us to another, more serious problem. In one of my favourite papers on structuring spreadsheets, Sarkar & Chaloub have this to say
...
Being able to move away from relative references should depend on how well these constraints can be met. In the same paper, Sarkar & Chaloub also look at other attempted solutions.
...
[^mitch-kapor]: https://www.youtube.com/watch?v=hcjwRFT_uto&t=1495s

[^replicate]: [Here's a short demo](https://youtu.be/wU8WhGtRWF8?feature=shared&t=126) of `REPLICATE` in action.

[^microsoft]: https://support.microsoft.com/en-gb/office/switch-between-relative-absolute-and-mixed-references-dfec08cd-ae65-4f56-839e-5f0d8d0baca9

[^multiplan]: Page 136 of https://vt100.net/dec/aa-p230a-tv_multiplan_reference.pdf

[^multiplan-refs]: Multiplan did not have A1-style referencing. It had R1C1 references only.

[^lotus]: Page 1-31 of https://ia601706.us.archive.org/25/items/lotus-1-2-3-release-3.1-reference/Lotus%201-2-3%20Release%203.1%20-%20Tutorial.pdf This is from a Lotus version 3.1. the earliest Lotus 1-2-3 manual I could find

[^boston-linux]: [Dan Bricklin and Bob Frankston's interview to Boston Linux](https://www.youtube.com/watch?v=hcjwRFT_uto&t=1495s), 2023

[^excel-2.0-online]: https://www.pcjs.org/software/pcx86/app/microsoft/excel/2.0/

[^ted-talk]: From [Meet the inventor of the electronic spreadsheet: Dan Bricklin](https://www.youtube.com/watch?v=YDvbDiJZpy0), 2017

[^implementing-visicalc]: From [Implementing Visicalc](https://rmf.vc/ImplementingVisiCalc) by Bob Frankston

[^elastic-sdf-example]: https://youtu.be/jH2Je6wUvPs?feature=shared&t=765

[^scan]: https://hoogle.haskell.org/?hoogle=scanl
